...
float Arg(vec2 z, int k) {
  return atan(z.y,z.x) + 2.*float(k)*pi;
}
float Abs(vec2 z) {
...
vec2 Sqrt(vec2 z, int k, float s) {
  float m = s/sqrt(2.0);
  float abs = Abs(z);
  return m*vec2(sqrt(abs+z.x),sign(z.y)*sqrt(abs-z.x));
}
// Exponential Functions
vec2 Exp(vec2 z) {
  return exp(z.x)*vec2(cos(z.y),sin(z.y));
}
vec2 Ln(vec2 z, int k) {
  return vec2(log(Abs(z)),Arg(z,k));
}
vec2 Log(vec2 w, vec2 z, int k) {
  return Div(Ln(z,k),Ln(w,k));
}
vec2 Pow(vec2 z, vec2 w, int k) {
  return Exp(Mult(w,Ln(z, k)));
}
vec2 Root(vec2 w, vec2 z, int k) {
...
// Inverse Trig
vec2 Arcsinh(vec2 z, int k, float s) {
  return Ln(z+Sqrt(Square(z)+vec2(1,0),k,s),k*2);
}
vec2 Arcsin(vec2 z, int k, float s) {
  return -Mult_i(Arcsinh(Mult_i(z),k,s));
}
vec2 Arccosh(vec2 z, int k, float s) {
  return Ln(z+Sqrt(Square(z)-vec2(1,0),k,s),k*2);
}
vec2 Arccos(vec2 z, int k, float s) {
  return -Mult_i(Arccosh(z,k,s));
}
vec2 Arctanh(vec2 z, int k) {
  return 0.5*Ln(Div(vec2(1,0)+z,vec2(1,0)-z),k);
}
vec2 Arctan(vec2 z, int k) {
  return -Mult_i(Arctanh(Mult_i(z),k));
}
vec2 Arccoth(vec2 z, int k) {
  return Arctanh(Inv(z),k);
}
vec2 Arccot(vec2 z, int k) {
  return Arctan(Inv(z),k);
}
vec2 Arcsech(vec2 z, int k, float s) {
  return Arccosh(Inv(z),k,s);
}
vec2 Arcsec(vec2 z, int k, float s) {
  return Arccos(Inv(z),k,s);
}
vec2 Arccsch(vec2 z, int k, float s) {
  return Arcsinh(Inv(z),k,s);
}
vec2 Arccsc(vec2 z, int k, float s) {
  return Arcsin(Inv(z),k,s);
}
// Misc
vec2 Gamma(vec2 z, int k) {
  vec2 prod = Inv(z);
...
void main() {
  ...
  float dx = length(#DEVAL#);
  float tolerance = dx/absR*u_zoom;
